$.widget( 'custom.mostrarRespuestas', {
	...
	_crearHeaderRespuesta: function(data) {
		...
		$usuarioDatos = $(' <div class="media-body"><h5 class="media-heading">'+sender.user+'<br/>Ubicacion : ' + ubicacion + '<br/>Fecha de ingreso: '+ fecha +
				'<br/>Fecha respuesta: '+data.fechaFormat+'</h5></div>');
	...
	_crearPanelTabla: function(data) {
		...
		$('<div align="center" class="respuesta-titulo" >'+data.tema.titulo+'</div>'+
				'<div>'+data.texto+'</div>').appendTo($panelBody);
		$('<div align="right" id="derecha">' +
				'<a class="link" href="tables.html"><i class="glyphicon glyphicon-comment"></i> Citar  </a>'+
				'<a class="link" href="tables.html"><i class="glyphicon glyphicon-thumbs-down"></i> Denunciar</a>')
